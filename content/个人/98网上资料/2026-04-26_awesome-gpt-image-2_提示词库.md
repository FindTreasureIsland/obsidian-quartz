...
date: 2026-04-26
tags: [AI绘图, GPT-Image, 提示词, Prompt]
---

# Awesome GPT-Image2 提示词库

> 项目愿景：把"散文式提示词"压缩成"结构化协议"，适合 Agent 和自动化工作流调用
> 
> 原文：[freestylefly/awesome-gpt-image-2](https://github.com/freestylefly/awesome-gpt-image-2)
...
1. 先在精选案例里确定要模仿的输出类型
2. 去完整画廊里找相近案例，**抄结构，不要只抄风格词**
3. 把业务变量填进通用模板或 JSON 模板

---

## 🧩 工业级提示词模板

来源：[docs/templates.md](./docs/templates.md)
...
**避坑指南：**
- 控制模块数量：强制定制"模块数量"和"图表类型"
- 文案克制：图表场景优先使用短句文案，不要把大段正文塞进画面
...
**避坑指南：**
- 材质和光影是灵魂：一定要堆叠材质（如"磨砂质感"）和灯光（如"轮廓光"）
- 别把促销贴满全屏：文案只给核心的 1-2 句
...
**避坑指南：**
- 做减法：先定义品牌关键词，再要求视觉输出
- 强制背景：必须强调"纯白背景（Pure White Background）"
...
**避坑指南：**
- 控制视角：用"Eye-level perspective（人眼视角）"压住透视变形
- 冷暖对比：室外冷光（蓝/灰）和室内暖光（黄/橙）搭配
...
**避坑指南：**
- 加点瑕疵：加入"皮肤纹理（skin pores）"、"轻微胶片颗粒（film grain）"
...
**避坑指南：**
- 锁定笔触：插画如果不限制笔触（如"厚涂"、"水彩晕染"），会变成 AI 默认塑料风
- 慎用大师名：提取大师特征（如"梵高的旋转星空笔触"），而不是直接写大师名
...
**避坑指南：**
- 拆解五官：不要只写"很美的女孩"，拆解成"桃花眼、高鼻梁、野生眉"
- 服装材质：写清衣服的材质（如"丝绸"、"机能防风面料"）
...
**避坑指南：**
- 要有"动词"：一定要写"事件"（如"正在崩塌"），让画面动起来
- 镜头语言：使用"Low angle shot"或"Dutch angle"增加戏剧冲突
...
**避坑指南：**
- 拒绝大杂烩：明确朝代（唐/宋/明），否则会穿帮
- 强制排雷：一定要加上"禁用现代元素（No modern elements）"
...
- 放弃全文：用"模拟文本（Simulated text blocks）"填充正文
...
来源：[docs/gallery-part-1.md](./docs/gallery-part-1.md) + [docs/gallery-part-2.md](./docs/gallery-part-2.md)
